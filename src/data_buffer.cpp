...
GL_DataBuffer<T>::GL_DataBuffer(uint32_t gl_buffer_type, uint32_t data_count, const T* buffer_data, uint32_t gl_buffer_usage) :
    m_gl_id(0), m_gl_buffer_type(gl_buffer_type), m_data_count(data_count), m_data_size(0), m_buffer_size(0)
{
...
template<typename T>
inline void GL_DataBuffer<T>::gl_bind() const
{
    GL_CALL(glBindBuffer(m_gl_buffer_type, m_gl_id));
}
...
template<typename T>
inline void GL_DataBuffer<T>::gl_unbind() const
{
